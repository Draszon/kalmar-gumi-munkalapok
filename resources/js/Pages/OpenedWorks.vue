...
const openEditModal = (worksheet) => {
  ...
  editForm.store = Boolean(worksheet.store);
...
const sortedWorksheets = computed(() => {
  let filtered = props.worksheets || [];
...
const closeWorksheet = (id) => {
  if (confirm('Biztosan lezárod ezt a munkalapot?')) {
    router.post(`/close-worksheet/${id}`);
  }
};

const deleteWorksheet = (id) => {
  if (confirm('Biztosan törölni szeretnéd ezt a munkalapot? Ez a művelet nem vonható vissza!')) {
    router.delete(`/delete-worksheet/${id}`);
  }
};
</script>
...
<template>
  <Head>
    <title>Nyitott munkalapok</title>
  </Head>
  ...
  <MainLayout>
    <section class="w-full flex justify-center py-8 px-4">
      <div class="w-full max-w-6xl">
        ...
        <!-- Header -->
        <div class="mb-8">
          <h1 class="text-2xl font-bold text-slate-800">Nyitott munkalapok</h1>
          <p class="text-slate-500 mt-1">Az összes folyamatban lévő munka áttekintése</p>
        </div>
          ...
          <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-6">
              <div class="flex items-center gap-2">
                ...
                <div>
                  <p class="text-2xl font-bold text-slate-800">{{ sortedWorksheets.length }}</p>
                  <p class="text-xs text-slate-500">Nyitott munkalap</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="space-y-3">
          <div 
            v-for="worksheet in sortedWorksheets" 
            :key="worksheet.id"
            class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden"
          >
            <!-- Kompakt fejléc - mindig látszik -->
            <div 
              class="p-4 flex items-center gap-4 cursor-pointer hover:bg-emerald-100 transition-colors bg-emerald-50 border-b-2 border-emerald-200"
              @click="toggleCard(worksheet.id)"
            >
              <!-- Rendszám -->
              <span class="font-mono font-bold text-base text-emerald-800 bg-white px-3 py-1.5 rounded-xl border border-emerald-300 uppercase shrink-0">
              ...
              <div class="hidden sm:block text-right shrink-0">
                <p class="text-xs text-slate-400">Létrehozva</p>
                <p class="text-sm font-medium text-slate-600">{{ formatDate(worksheet.created_at) }}</p>
              </div>
            ...
            >
              <div class="overflow-hidden">
                <!-- Akciógombok -->
                <div class="px-4 pb-3 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3">
                  <!-- Szerkesztés gomb -->
                  <button 
                  ...
                  <!-- Törlés gomb -->
                  <button 
                    @click.stop="deleteWorksheet(worksheet.id)"
                    class="flex items-center gap-2 px-3 py-2 rounded-xl text-red-600 bg-red-50 hover:bg-red-100 border border-red-200 font-medium transition-colors text-sm"
                    title="Törlés"
                  >
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <!-- Lezárás gomb -->
                  <button 
                    @click.stop="closeWorksheet(worksheet.id)"
          ...
          <!-- Üres állapot -->
          <div v-if="sortedWorksheets.length === 0" class="bg-white rounded-2xl shadow-sm border border-slate-100 p-16">
            <div class="flex flex-col items-center">
              ...
              <p class="text-slate-600 font-medium mb-1">Nincs nyitott munkalap</p>
              <p class="text-slate-400 text-sm mb-4">Hozz létre egy újat a kezdéshez</p>
              <a 
                href="/"
                class="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-600 text-white font-medium rounded-xl transition-colors"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
